...
namespace VirtualStore_RP.BLL
{
    public class ProviderBLL
    {
        ...
        {
            string sql = string.Format($@"UPDATE provider SET name='{provider.Name}', cnpj='{provider.Cnpj}', email='{provider.Email}', phone='{provider.Phone}', representativeName='{provider.RepresentativeName}', representativePhone='{provider.RepresentativePhone}';");
            connection.ExecutionSQL(sql);
        }
